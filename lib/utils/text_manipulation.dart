...
String addBookLinks(String text) {
  ...
  // תמיכה בקישורים עם דפים ספציפיים (לדוגמה: "ברכות דף ב" או "ברכות ב")
  final pagePattern = RegExp(
    r'\b(' + bookLinks.keys.map(RegExp.escape).join('|') + r')\s+(?:דף\s+)?([א-ת]{1,3})(?:\s+עמוד\s+([אב]))?\b'
  );
  
  result = result.replaceAllMapped(pagePattern, (match) {
    final bookName = match.group(1)!;
    final page = match.group(2)!;
    final side = match.group(3) ?? '';
    final bookTitle = bookLinks[bookName]!;
    
    // יצירת קישור עם דף ספציפי
    return '<a href="book://$bookTitle#דף_$page$side">${match.group(0)}</a>';
  });
  
  // עבור כל ספר ברשימה, נחליף את המילה בקישור (ללא דף ספציפי)
  bookLinks.forEach((word, bookTitle) {
    // נוודא שאנחנו מחליפים רק מילים שלמות שלא כבר הפכו לקישור
    final regex = RegExp(r'(?<!href="book://[^"]*)\b' + RegExp.escape(word) + r'\b(?![^<]*</a>)');
